...
{
	m_BallTexture = new OpenGLTexture("ballTexture");
}
...
{
	setSpeed(getSpeed() + GAME_BALL_SPEED_INCREASE_RATE);
...
void Ball::paint()
{
	...
	OpenGLRenderer::getInstance()->drawTexture(m_BallTexture, getX() - getRadius(), getY() - getRadius(), getRadius()*2, getRadius()*2);
...
{
	if(getSpeed() <= GAME_BALL_MAX_SPEED)
	{
		m_Speed = aSpeed;
	}
}
...
{
	m_Speed = GAME_BALL_DEFAULT_SPEED;
}
