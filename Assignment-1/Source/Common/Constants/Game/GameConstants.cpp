#include "GameConstants.h"


//Game Constants
const char* GAME_SCREEN_NAME = "Game";
const double GAME_OVER_TIMER = 2.5;

//Ball Constants
const float GAME_BALL_RADIUS_PERCENTAGE = 0.02f;
const float GAME_BALL_DEFAULT_SPEED = 250.0f;
const float GAME_BALL_MAX_SPEED = 1500.0f;
const float GAME_BALL_SPEED_INCREASE_RATE = 0.1;
const char* GAME_BALL_TYPE = "Ball";

//Paddle Constants
const float GAME_PADDLE_WIDTH_PERCENTAGE = 0.2f;
const float GAME_PADDLE_HEIGHT_PERCENTAGE = 0.05f;
const float GAME_PADDLE_Y_PERCENTAGE = 0.9f;
const char* GAME_PADDLE_TYPE = "Paddle";

//Brick Constants
const float GAME_BRICK_WIDTH_PERCENTAGE = 0.083f;
const float GAME_BRICK_HEIGHT_PERCENTAGE = 0.050f;
const float GAME_BRICK_Y_PERCENTAGE = 0.005f;
const float GAME_BRICK_X_PERCENTAGE = 0.100f;
const char* GAME_BRICK_TYPE = "Brick";
